...
public class AccountControllerIntegrationTest {
  ...
  void getAccount_unknownId_returns404() throws Exception {
    mockMvc
        .perform(get("/accounts/{id}", UUID.randomUUID()).with(jwt()))
  ...
  void updateAccount_unknownId_returns404() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/accounts/{id}", UUID.randomUUID())
                .with(jwt().authorities(new SimpleGrantedAuthority("ROLE_ADMIN")))
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
        .andExpect(status().isNotFound());
  ...
  void updateAccount_validRequest_returns200WithUpdatedFields() throws Exception {
    ...
    var body =
        objectMapper.writeValueAsString(new UpdateAccountRequest("After", AccountStatus.FROZEN));
    mockMvc
        .perform(
  ...
  void getAllAccounts_returnsPagedResultsFilteredByStatusAndOwnerName() throws Exception {
    ...
    var close =
        objectMapper.writeValueAsString(new UpdateAccountRequest(null, AccountStatus.CLOSED));
    var closedId = createAccount("Closed Carl");
    mockMvc
        .perform(
            patch("/accounts/{id}", closedId)
                .with(jwt().authorities(new SimpleGrantedAuthority("ROLE_ADMIN")))
                .contentType(MediaType.APPLICATION_JSON)
                .content(close))
        .andExpect(status().isOk());
