...
public class AccountController {
  ...
  @Operation(
      ...
      description =
          "Supports pagination (page, size), sorting (sort), and filtering by status and ownerName.")
  ...
  public PagedModel<AccountResponse> getAllAccounts(
    ...
    Page<AccountResponse> page =
        accountService.getAll(status, ownerName, pageable).map(AccountResponse::from);
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "409",
        description = "Account is closed and cannot be modified",
        content = @Content(schema = @Schema(implementation = ProblemDetail.class)))
  })
