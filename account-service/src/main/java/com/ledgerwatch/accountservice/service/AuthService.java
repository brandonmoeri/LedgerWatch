...
/**
 * Dev-only credential store and token issuer. There is no user database yet; this exists so the
 * frontend has a real login flow to hit instead of a stub. Swap the in-memory {@code users} map for
 * a real user store when one exists.
 */
@Service
  ...
  public AuthService(@Value("${jwt.secret}") String jwtSecret, PasswordEncoder passwordEncoder) {
    ...
    this.users =
        Map.of(
            ...
            "analyst",
                new DevUser("analyst", passwordEncoder.encode("analyst123"), List.of("ANALYST")));
  }
