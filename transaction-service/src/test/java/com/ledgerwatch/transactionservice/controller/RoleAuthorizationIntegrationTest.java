...
class RoleAuthorizationIntegrationTest {
  ...
  void noToken_postTransactions_returns401() throws Exception {
    var body =
        objectMapper.writeValueAsString(
            new CreateTransactionRequest(
                UUID.randomUUID(), TransactionType.CREDIT, new BigDecimal("50.00"), null));

    mockMvc
        .perform(post("/transactions").contentType(MediaType.APPLICATION_JSON).content(body))
  ...
  void analystRole_patchTransactions_returns403() throws Exception {
    ...
    var body =
        objectMapper.writeValueAsString(new UpdateTransactionRequest(null, "attempted edit"));
