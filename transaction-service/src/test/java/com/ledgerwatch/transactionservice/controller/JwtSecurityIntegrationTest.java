...
public class JwtSecurityIntegrationTest {
  ...
  void validSignedToken_returns200() throws Exception {
    mockMvc
        .perform(get("/transactions").header("Authorization", "Bearer " + mintToken(jwtSecret)))
  ...
  void tokenSignedWithWrongSecret_returns401() throws Exception {
    mockMvc
        .perform(
            get("/transactions")
                .header(
                    "Authorization", "Bearer " + mintToken("wrong-secret-wrong-secret-wrong!!")))
  ...
  void tamperedSignature_returns401() throws Exception {
    ...
    mockMvc
        .perform(get("/transactions").header("Authorization", "Bearer " + tampered))
        .andExpect(status().isUnauthorized());
  }
